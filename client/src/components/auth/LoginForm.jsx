...
export default function LoginForm({ onSuccess, switchToRegister }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      {/* Heading */}
      <div className="mb-2">
        <h2 className="text-xl font-semibold text-[var(--text-primary)] tracking-tight">Welcome back</h2>
        <p className="text-sm text-[var(--text-secondary)] mt-1">Sign in to continue your journey</p>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            className="text-xs text-[var(--text-link)] hover:underline transition-colors cursor-pointer border-none bg-transparent font-medium"
          >
            Forgot password?
          </button>
        </div>
      ...
      <div className="flex items-center gap-3 my-1">
        ...
        <span className="text-[10px] text-[var(--text-tertiary)] font-medium uppercase tracking-wider">or continue with</span>
        <div className="flex-1 h-px bg-[var(--border-default)]" />
      </div>
